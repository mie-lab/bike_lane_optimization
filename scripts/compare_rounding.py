        ...
        for optimize_every in OPTIMIZE_EVERY_LIST:
            for rounding_method in ROUNDING_METHOD:
                for number_paths in NUMBER_PATHS_FOR_PRUNING:
                    ...
                    pareto_front.to_csv(
                        os.path.join(OUT_PATH, f"pareto_optimize{CAR_WEIGHT}_{graph_trial}_{rounding_method}_{optimize_every}_{number_paths}.csv")
                    )

    # -------- PLOTTING -------------
    # this code loads all the result files and plots them
    # set the color maps
    colors = plt.cm.viridis(np.linspace(0, 1, len(NUMBER_PATHS_FOR_PRUNING)))
        ...
        for i, optimize_every in enumerate(OPTIMIZE_EVERY_LIST):
            for j, rounding_method in enumerate(ROUNDING_METHOD):
                for k, number_paths in enumerate(NUMBER_PATHS_FOR_PRUNING):
                    ...
                    plt.plot(pareto_front["bike_time"], pareto_front["car_time"], label=str(optimize_every) + " p " + str(number_paths) + " " + rounding_method, c=colors[k], ls=styles[j])
