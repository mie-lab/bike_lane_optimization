...
def betweenness_pareto(G_lane, od_matrix=None, sp_method="all_pairs", shared_lane_factor=2, weight_od_flow=False):
    ...
    # start with car graph = full graph
    car_graph = G_lane.copy()
    # save nodes for bike graph later
    node_attributes = nx.get_node_attributes(car_graph, name="loc")
    # init is_fixed
    is_fixed = {edge: False for edge in car_graph.edges}
    ...
    # iteratively recompute betweenness centrality
    while iters < max_iters:
        # check if all edges are fixed -> break
        if np.all(np.array(list(is_fixed.values()))):
            print("All edges are fixed, break")
            break

        betweenness = nx.edge_betweenness_centrality(car_graph)
        # find edge with lowest betweenness centrality that it not fixed
        sorted_edges = sorted(betweenness.items(), key=lambda x: x[1])
        for s in sorted_edges:
            if not is_fixed[s[0]]:
                ...
                break
        # remove this edge
        car_graph.remove_edge(*min_edge)
        if not nx.is_strongly_connected(car_graph):
            car_graph.add_edge(*min_edge)
            is_fixed[min_edge] = True
            if all(is_fixed.values()):
                break
        else:
            ...
            # transform into bik_G and car_G and compute travel times
            ...
            travel_time_dict["bike_edges_added"] = edges_removed
